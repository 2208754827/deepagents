...
```json
// ✅ 正确：操作列仅含用户动作，预期列仅含系统断言
{"操作": "1. 打开登录页面", "预期": "1. 显示账号输入框、密码输入框和登录按钮"}
{"操作": "2. 输入正确的账号和密码", "预期": "2. 账号和密码正确填入"}
{"操作": "3. 点击登录按钮", "预期": "3. 登录成功，跳转到首页，显示用户昵称"}

// ❌ 错误：操作列混入预期结果（严禁）
{"操作": "1. 打开登录页面 -> 显示账号输入框、密码输入框", "预期": "1. 显示账号输入框、密码输入框"}

// ❌ 错误：前置数据准备混入步骤数组（严禁）
// 验证目标：登录失败后账号锁定 → 前置条件应为"连续输入错误密码4次"
// 而非在步骤数组中写"1. 连续输入4次错误密码"作为"准备步骤"
```
